...
The three are named rather than iconed, because no drawing distinguishes them and telling them apart is the point. The chosen one carries the selected surface and a border, not a colour alone. A message that cannot be drawn the chosen way falls through to one that can, and the choice itself does not move, because it still stands for the next message. Too heavy to lay out says so in a notice: what arrives is text nobody asked for, and that needs a reason. Nothing readable in it does not, because what it falls through to is the sender's own layout, which is the whole message and explains itself.
