...
|---|---|
| ~~The From address the composer picked is discarded on the wire.~~ | Fixed. `ComposeParts` carries `from`, and the host refuses an address the mailbox may not send as *before* reading a credential — the window is the thing being distrusted, so the check cannot live there. A draft written elsewhere under a send-as alias is now refused rather than silently sent as the account, which is the honest half of the alias row still open below |
| ~~A forward goes out without the original's attachments.~~ | Fixed. `app/application/compose-attachments.js` fetches the parts, keeps them through `omamail-attachment.store` (the send path opens by path; bytes cannot cross under `MAX_REQUEST_BYTES`), and Send is held until they arrive |
| ~~An IMAP action the server refused is reported as done.~~ | Fixed in Rust rather than in `ImapProtocol.isFailure`, because the action path reads `result.ok` and never the response text: a JS check would have to be repeated at every call site. `tagged_failure` steps over literals by their octet count, so `X-Spam-Flag: NO` in a header is not read as protocol |
| ~~A message read a moment ago comes back bold, and a trashed row reappears.~~ | Fixed. The optimistic edit is written back through `cache.writeList`, and the in-flight list read is interrupted first — the two halves `MailAccount.qml:1310` and `:1452` document |
| ~~A draft saved with files, reopened and sent, goes out without them.~~ | Fixed, and the strip is drawn rather than hidden: the QML hides it, which leaves Send held with nothing said about why |
| Leaving the composer on IMAP or HEY discards the draft with no warning. | Still open. The host has no draft save for either — `groupware.rs` returns `Unsupported` for IMAP, and HEY's compose command takes only a reply or a forward |

## The core loop

| Capability | Status | Where |
|---|---|---|
| ~~Opening an unread message marks it read~~ | done | `markOpenedRead` runs from both branches of `openCursor` — a cached body skips the fetch entirely, so it cannot live only on the fetch path |
...
| ~~Switching accounts keeps the mailbox you were in~~ | done | `activate` takes the mailbox to keep |
...
| ~~Search submits on Enter and closes the open message~~ | done | `app/application/search.js`: a 250 ms debounce on change, an immediate path on submit, an in-flight guard, and the reader closes |

## Whole features that never run

| Capability | Status | Where |
|---|---|---|
| ~~The unsubscribe notice, and unsubscribing~~ | done | Two adapter lines lit the whole path. All three plans work, and a sender-controlled address is re-judged before it is acted on — a cached record is not trusted for having come off disk |
| Calendar invitations | **display done, RSVP open** | Both adapters produce `invite`; Gmail chains one attachment fetch when Google described the part instead of sending it. `canRespond` is deliberately hard `false` — the card shows the meeting and offers no button that would fail. See "Finishing RSVP" below |
...
| ~~The status line confirms an action~~ | done | `actionLabel` ported into `mail-state.js`; the notice is kept apart from the error and retired by the clock that already beats the compose toast, not a second one |
...
| ~~Clicking a row in Drafts resumes the draft~~ | done | both paths go through `openCursorDetail` |
...
Answering an invitation is an outgoing message, not a calendar API call — the
QML sends an RFC 5546 REPLY through `Message.buildSendPayload({calendar})` and
needs no second scope. Six edits, in the order they unblock each other:

1. `src/providers/groupware.rs` — `ComposeDraft` gains `calendar: Option<String>`,
   validated (`BEGIN:VCALENDAR` prefix, a size cap) and returned by `into_parts()`.
2. `src/native_groupware_runtime.rs` — `message()` emits
   `text/calendar; method=REPLY; charset=UTF-8`, base64, as a
   `multipart/alternative` sibling of the text part.
3. `src/imap_host.rs` — the same part on `MailOperation::SendThreaded`.
4. `app/main.js` — `hostRequestFor`'s `compose.send` branch passes
   `draft.calendar` under the same two checks.
5. `app/application/reader-model.js` — `canRespond: canSend &&
   Calendar.canRespond(invite, answeringAs)`, and an `onRsvp` sending
   `Calendar.replyFields(...)` with the message's `inReplyTo` / `threadId`.
6. `app/application/controller.js` — `rememberResponse`: write
   `Calendar.withResponse(invite, answeringAs, answer)` back through
   `rememberBody` and refresh `detail`, so the card and the cached file agree.
