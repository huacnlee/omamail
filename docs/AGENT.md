...
With **Suggest calendar events from mail** on in Settings — and only then — a message opened in the reader whose text mentions a date or a time is handed to the default agent once, in the background, with rules that ask for a JSON array of the events it finds and nothing else. The runner reads the array out of the answer (`agent-job.py` keeps the events on the job, as epoch milliseconds, whole days included) and the reader draws a card per event above the message: what, when, where, with **Add** and **Dismiss**. Add opens the calendar's event composer with the fields filled in, so the owner chooses the calendar and looks the times over before anything is written; a written event waves its suggestion away, a dismissed one stays away for the session.

The gates are all local and cheap: the setting, an agent, a date in the text, a message from the last two months (one with no known date is not looked at), no look at that message yet, and at most two looks running at once; a look the runner cannot start yet waits its turn. A look is a job like any other in the pane, but a background one — no row glyph, no glow, and a note only when it found something. The ask stands above the message in the prompt, every line of the message is prefixed with `| ` between two fence lines, and the rules say those lines are a stranger's words, not instructions; the runner exports `OMAMAIL_JOB_KIND=events` so a wrapper can run a look with fewer tools than an ask the owner typed. That is a request to the model and a hook for the harness, not a sandbox: the agent command is the one you trust to read a stranger's mail, which is why the setting is off until you turn it on. A look runs the **Agent for background looks** command, which left empty is the default agent's harness at its cheapest model with no tools, where a preset knows it — Claude Code at `claude-haiku-4-5-20251001` with no tool allowed without asking (which in print mode means none runs), Codex at `gpt-5.1-codex-mini` in the read-only sandbox, Gemini at `gemini-2.5-flash` without `--yolo` — matched by the program the default agent runs, whatever flags follow it; a harness no preset knows runs as typed, and a line typed in the field runs instead. The model ids are the ones the CLIs took when this was written; the field is where a newer one goes.
