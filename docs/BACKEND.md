...
Native message preparation decodes supported MIME and RFC 2047 charsets with `encoding_rs`, including Windows-1251 and GB2312. Transfer-decoded attachment and resource bytes stay unchanged; only displayed text is converted to Unicode. The existing strict UTF-8 check still repairs UTF-8 sent under a single-byte label. Malformed known legacy sequences become replacement characters; UTF-8 and unknown labels retain the existing permissive fallback. Charset decoding happens before HTML sanitization, never instead of it.
