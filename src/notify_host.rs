...
/// Hands the vector to `notify-send` and does not wait for it.
///
...
/// and a mail read must not be the thing that discovers that.
pub struct NotifySendLauncher;
impl NotificationSender for NotifySendLauncher {
    fn send(&self, arguments: &[String]) -> Result<(), NotifyError> {
        let child = Command::new(NOTIFY_SEND)
            .args(arguments)
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
...
fn arguments(summary: &str, body: &str, icon: Option<&Path>) -> Vec<String> {
