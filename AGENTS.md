...
- **Choose the title opening from the final change.** Bug fixes start with `Fix ` (a space, not a colon), including fixes to AI features, documentation, or the website: `Fix AI command deletion`. Otherwise, AI feature changes use `ai: `, documentation-only changes use `docs: `, and website changes use `website: `, for example `ai: Show slash commands as highlighted tokens`, `docs: Document backend releases`, and `website: Add provider setup examples`. Website content changes use `website: `; `docs: ` is for repository documentation. Other changes use an unprefixed imperative title. For mixed changes, use the primary outcome rather than stacking prefixes; supporting documentation does not turn a feature PR into a documentation PR.
