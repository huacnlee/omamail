...
- **UID order is not date order.** Proton Bridge can assign imported messages UIDs newest-first. Native listing takes a compact `UID FETCH 1:* (UID)` snapshot, fetches INTERNALDATE in explicit UID batches of at most 4096 messages, searches stable UID windows, and sorts matching UIDs by arrival date before paging. Descending UID is only a deterministic date tie-breaker. Never return a high-UID window as a settled newest prefix, even when it fills a page, and never replace UID windows with sequence-number windows that move when another client expunges mail. Unsolicited UID/FLAGS updates must not erase a previously fetched date.
