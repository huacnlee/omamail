...
Item {
  ...
  // Whose message: the account open when the popup opened, by id, stamped
  // from the service as the popup opens. The ask, the cancel and the job it
  // shows are read inside that account, so a switch under the popup cannot
  // turn Ada's ask into one about Bob's row of the same id.
  property var service: null
  property string accountId: ""
  function stampOwner() { accountId = service ? String(service.activeAccountId || "") : "" }
