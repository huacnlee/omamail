...
// Composing is its own window, not a column in the mailbox. Hyprland tiles it
// beside the main window, so the message being answered stays on screen
// instead of being covered by the form.
//
...
Item {
  ...
  FloatingWindow {
    id: window
    visible: root.opened
    title: root.mode === "new" ? "New message — Omarchy Gmail"
      : "Reply — Omarchy Gmail"
    color: root.backgroundColor
    implicitWidth: Style.space(720)
    implicitHeight: Style.space(560)
    minimumSize: Qt.size(Style.space(480), Style.space(380))

    onVisibleChanged: if (!visible && root.opened) root.finish()
